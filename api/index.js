...
app.post('/api/groups', auth, async (req, res) => {
  try {
    let { tid, group, lang, time, start, exam, students, level, doneInLevel } = req.body;
    ...
    const g = await Group.create({ tid, group, lang, time, start, exam, students: +students, level, doneInLevel });
...
app.put('/api/groups/:id', auth, async (req, res) => {
  try {
    ...
    for (const f of ['group', 'lang', 'time', 'start', 'exam', 'students', 'level', 'doneInLevel']) if (req.body[f] != null) g[f] = req.body[f];
